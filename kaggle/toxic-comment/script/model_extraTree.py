...
def run_cv():
    ...
    for fold in range(K):
        ...
        df_preds_test.to_csv('../data/output/preds/extraTree/{0}fold_test.csv'.format(fold), index=False)
        ...
        df_preds_val.to_csv('../data/output/preds/extraTree/{0}fold_valid.csv'.format(fold), index=False)
