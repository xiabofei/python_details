...
# asterisk mask toxic words
asterisk_mask = {
    'fuck': ['fwk'],
    'fuckin': ['f**kin', 'f**in', 'f***in', 'fuc**n', ],
    'fucker': ['f***er', 'f****er', 'f**ker', 'f**kker'],
    'fuckers': ['f**kers', ],
    'fucked': ['f**ked', ],
    'fuckhead': ['f**khead', ],
    'headfuck': ['headf**k', ],
    'fucking': ['fwcking', 'fu**ing', 'f**cking', 'f**king', ],
    'motherfucking': ['motherf**king', ],
    'motherfucker': ['motherf**ker', ],
    'motherfuckers': ['motherf**kers', 'mother****ers', ],
    'motherfuckin': ['motherf***in', 'moderf***n', ],
    'shithead': ['s**thead', ],
    'bullshit': ['bull****', 'bulls***', 'bulls**t', ],
    'cunt': ['c**t', ],
    'bitch': ['b**ch', 'bi*ch', ],
    'asshole': ['a**hole', 'as**ole', 'a**h**e', 'a*****e'],
    'assholes': ['a**holes', '****holes', '***holes', ],
    'shithole': ['shit**oles', ],
    'jackass': ['jack***', ],
...
class CommProcess(object):
    ...
    def revise_star(t):
        ret = []
        for word in t.split():
            # remove '*' then revise to complete toxic format
            if ('*' in word) and (re.sub('\*', '', word) in toxic_indicator_words_sets):
                word = re.sub('\*', '', word)
            # apply asterisk revise
            if ('*' in word) and (word in masks):
