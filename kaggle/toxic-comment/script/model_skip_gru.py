...
def get_model(embedding_lookup_table, dropout, spatial_dropout):
    ...
    layer = concatenate([gru_two, gru_one])
...
def run_one_fold(fold):
    ...
    for run in range(RUNS_IN_FOLD):
        ...
        bst_model_path = \
            '../data/output/model/{0}fold_{1}run_{2}dp_{3}sdp_skip_gru.h5'.format(fold, run, FLAGS.dp, FLAGS.sdp)
    ...
    df_preds_test.to_csv(
        '../data/output/preds/skip_gru/{0}/{1}/{2}fold_test.csv'.format(FLAGS.dp, FLAGS.sdp, fold), index=False)
    ...
    df_preds_val.to_csv(
        '../data/output/preds/skip_gru/{0}/{1}/{2}fold_valid.csv'.format(FLAGS.dp, FLAGS.sdp, fold), index=False)
