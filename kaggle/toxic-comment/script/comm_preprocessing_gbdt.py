...
data_comm_preprocessed_dir = '../data/input/data_comm_preprocessed_gbdt/'
...
# transform toxic word abbr to complete format
toxic_complete_abbrs = {
    'fuck':[
        'fuck', 'fucking', 'fucked', 'fuckin', 'fucka', 'fucker', 'fucks', 'fuckers',
        'fck', 'fcking', 'fcked', 'fckin', 'fcker', 'fcks',
        'fuk', 'fuking', 'fuked', 'fukin', 'fuker', 'fuks', 'fukers',
...
abbrs_complete = dict()
for complete, abbrs in toxic_complete_abbrs.items():
    for abbr in abbrs:
        abbrs_complete[abbr] = complete
toxic_abbrs_sets = set(abbrs_complete.keys())
...
class CommProcess(object):
    ...
    @staticmethod
    def trans_abbrs_to_complete(t):
        ret = []
        for word in t.split():
            if word in toxic_abbrs_sets:
                word = abbrs_complete[word]
            ret.append(word)
        return ' '.join(ret)
    ...
    @staticmethod
    def stemmer(t):
        return ' '.join([english_stemmer.stem(w) for w in t.split()])
...
def execute_comm_process(df):
    comm_process_pipeline = [
        ...
        CommProcess.trans_abbrs_to_complete,
