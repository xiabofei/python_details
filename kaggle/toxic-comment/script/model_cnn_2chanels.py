...
def get_model(glove_embedding_lookup_table, dropout):
    ...
    ## Chanel 1 : not trainable embedding
    not_trainable_embedding_layer = Embedding(
    ...
    not_trainable_layer_sdp = SpatialDropout1D(0.2)(not_trainable_embedding_layer)
    kernel_sizes = [2, 3, 4]
    num_filters = 128
    # num_filters = vibration_filters
    glove_not_trainable = []
    for kernel_size in kernel_sizes:
        conv1d = Conv1D(filters=num_filters, kernel_size=kernel_size, activation='relu')(not_trainable_layer_sdp)
        bn = BatchNormalization()(conv1d)
        max_pool = GlobalMaxPooling1D()(bn)
        # max_pool = AttentionWeightedAverage()(bn)
        glove_not_trainable.append(max_pool)

    ## Chanel 2 : trainable embedding
    ...
    trainable_layer_sdp = SpatialDropout1D(0.2)(trainable_embedding_layer)
    kernel_sizes = [2, 3, 4]
    num_filters = 128
    glove_trainable = []
    for kernel_size in kernel_sizes:
        conv1d = Conv1D(filters=num_filters, kernel_size=kernel_size, activation='relu')(trainable_layer_sdp)
        bn = BatchNormalization()(conv1d)
        # max_pool = GlobalMaxPooling1D()(bn)
        max_pool = AttentionWeightedAverage()(bn)
        glove_trainable.append(max_pool)
    layer = concatenate(glove_not_trainable + glove_trainable)
