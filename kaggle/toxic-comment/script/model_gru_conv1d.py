...
def get_model(embedding_lookup_table, spatial_dropout):
    ...
    layer = Conv1D(64, kernel_size=2, padding='valid', kernel_initializer='he_uniform')(layer)
...
def run_one_fold(fold):
    ...
    for run in range(RUNS_IN_FOLD):
        ...
        bst_model_path = \
            '../data/output/model/{0}fold_{1}run_{2}sdp_gru_conv1d.h5'.format(
                fold, run, FLAGS.sdp)
    ...
    df_preds_test.to_csv(
        '../data/output/preds/gru_conv1d/{0}/{1}fold_test.csv'.format(FLAGS.sdp, fold), index=False)
    ...
    df_preds_val.to_csv(
        '../data/output/preds/gru_conv1d/{0}/{1}fold_valid.csv'.format(FLAGS.sdp, fold), index=False)
