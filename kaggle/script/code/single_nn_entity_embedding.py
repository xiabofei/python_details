...
skf = StratifiedKFold(n_splits=K, random_state=231, shuffle=True)
# skf = comm_skf
...
for i, (f_ind, outf_ind) in enumerate(skf.split(X_train, y_train)):
    ...
    # twice positive upsampling examples
    X_train_f = pd.concat([X_train_f, X_train_f.loc[pos], X_train_f.loc[pos]], axis=0)
    y_train_f = pd.concat([y_train_f, y_train_f.loc[pos], y_train_f.loc[pos]], axis=0)
    ...
    for j in range(runs_per_fold):
        ...
        # logging.info('NN info describe : {0}'.format(NN.summary()))
        # bst_model_path = './tmp/single_nn_{0}.h5'.format(K)
        ...
        bst_score_epoch = np.argmin(hist.history['val_loss'])
        bst_score_value = hist.history['val_loss'][bst_score_epoch]
        logging.info('Fold {0} Run {1}, best val-loss {2}, at epoch {3}'.format(i, j, bst_score_value, bst_score_epoch))
    ...
    logging.info('Fold {0} prediction cv gini: {1}'.format(i, cv_gini))

logging.info('Mean out of fold gini: {0}'.format(np.mean(cv_ginis)))
logging.info('Full validation gini: {0}'.format(gini_normalizedc(y_train.values, full_val_preds)))
...
df_sub = pd.DataFrame({'id': df_test.id,
                       'target': y_pred_final},
                      columns=['id', 'target'])
random_diff = np.random.randint(0,10000)
logging.info('random diff {0}'.format(random_diff))
df_sub.to_csv('NN_EntityEmbed_{0}fold{1}-sub.csv'.format(K, random_diff), index=False)

pd.DataFrame(full_val_preds).to_csv('NN_EntityEmbed_{0}fold{1}-val_preds.csv'.format(K, random_diff), index=False)